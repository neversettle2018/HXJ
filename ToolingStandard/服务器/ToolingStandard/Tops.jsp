...
<body>
	...
	<div class="banner-e">
		...
		<div class="banner-e-1">
			<ul>
				<c:forEach var="t" items="${pageBean2.list}">
	...
	<div class="fenye">
		<c:choose>
			<c:when test="${pageBean2.pageNow==1}">
				<a>首页</a>
				<a>上一页</a>
			</c:when>
			<c:otherwise>
				<a href="tops?pageNow=1">首页</a>
				<a href="tops?pageNow=${pageBean2.pageNow-1}">上一页</a>
			</c:otherwise>
		</c:choose>
	
		<!--var定义 -->
		<c:forEach var="i" step="1" begin="1" end="${pageBean2.pageCount}">
			<c:choose>
				<c:when test="${pageBean2.pageNow eq i}">
	        [${i }]
	        </c:when>
				<c:otherwise>
					<a href="tops?pageNow=${i}">[${i }]</a>
				</c:otherwise>
			</c:choose>
		</c:forEach>
		<c:choose>
			<c:when test="${pageBean2.pageNow eq pageBean2.pageCount}">
				<a>下一页</a>
				<a>尾页</a>
			</c:when>
			<c:otherwise>
				<a href="tops?pageNow=${pageBean2.pageNow+1}">下一页</a>
				<a href="tops?pageNow=${pageBean2.pageCount}">尾页</a>
			</c:otherwise>
		</c:choose>
	
		&nbsp;共有
		<a>${pageBean2.rowCount}</a>条记录
